...
export class RealtimeAPI extends RealtimeEventHandler {
   ...
   * @param {{url?: string, apiKey?: string, dangerouslyAllowAPIKeyInBrowser?: boolean, debug?: boolean, azureConfig?: {endpoint?: string, deploymentId?: string}}} [settings]
   * @returns {RealtimeAPI}
   */
  constructor({ url, apiKey, dangerouslyAllowAPIKeyInBrowser, debug, azureConfig } = {}) {
    super();
  ...
  async connect({ model } = { model: 'gpt-4o-realtime-preview-2024-10-01' }) {
    ...
    // 构建 Azure WebSocket URL
    let wsUrl = this.url;
    if (this.azureConfig) {
      const { endpoint, deploymentId } = this.azureConfig;
      wsUrl = `wss://${endpoint}/openai/deployments/${deploymentId}/realtime?api-version=2024-02-15-preview`;
    }
